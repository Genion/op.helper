	...
	class OP {
	    ...
	    post(path, obj, cb, fail) {
	        ...
	        if (this.cookie) {
	            opts.headers['Cookie'] = this.cookie.value.join('; ');
	        }
	    ...
	    process() {
	        ...
	        this.post('/cloud/open', data, (resp) => {
	            if (resp.indexOf('impossible to create a VM Plan 01') == -1) {
	                log('Create success , enjoy!');
	                //location.href = url + 'cloud'
	            } else {
	                log('Create fail' + ' ( ' + ++this.count + ' times )');
	                setTimeout(() =>{
	                this.process();
	            }, 50);
	            }
	        }, (e) => {
	            log('Request fail' + ' (' + this.count + ')');
	            setTimeout(() =>{
	                this.process();
	            }, 50);
	        })
